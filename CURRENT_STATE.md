...
## What's in place
- ✅ `docs/index.html`: countdown hero (deadline 2026-09-26 23:59:59 Pacific, in `data-deadline`), welcome popup (`<dialog>`, once per browser session) with a mailing-list email field that posts to a Google Form and lands in the Sheet "Draper Fellowship mailing list (Responses)" in the user's Drive (IDs in `project_log.md`), at-a-glance stats, program pillars, Tim Draper portfolio logo marquee (`docs/logos/`, grayscale with color on hover), race-themed timeline, funding band, before-you-apply checklist with the Apply button (Google Form: https://forms.gle/SGNK3WYaj53eyeUG7).
